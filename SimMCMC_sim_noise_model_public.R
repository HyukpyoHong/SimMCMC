...
# setwd("C:/Users/Jayson/Desktop/Hyukpyo_code/")
...
for(jj in 1:nsample){
  ...
  myList <- TimeDelayGillespieforXY(A.X = A.X, B.X = B, alpha.X = alpha.X, beta.X = beta.X, A.Y = A.Y, B.Y = B, alpha.Y = alpha.Y, beta.Y = beta.Y, K.M = K.M, repnum = max.T*10000, maxT = max.T+3)
  ...
  obs.Y.all[2:(max.T+1),jj] = sim.Y.all[2:(max.T+1),jj] + sqrt(var.noise)*rnorm(max.T) + sqrt(sim.Y.all[2:(max.T+1),jj])*rnorm(max.T)
}
...
save.image(file = paste("Estimation_results.RData", sep = ""))

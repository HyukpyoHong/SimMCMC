...
myList <- TimeDelayGillespieforXY(A.X = A.X, B.X = B.X, alpha.X = alpha.X, beta.X = beta.X, A.Y = A.Y, B.Y = B.Y, alpha.Y = alpha.Y, beta.Y = beta.Y, K.M = K.M, repnum = max.T*70, maxT = max.T+3)
...
tun.X = seq(from=1, by=0.05, length.out=length(Y)) # tunning parameter for the setting 1.
...
for (rep in 1:nrepeat) {
  ...
  for(jj in 1:gen.num){
    myList <- TimeDelayGillespieforX(A.X = theta.X[1], B.X = theta.X[2], alpha.X = Delta.X[1], beta.X = Delta.X[2], repnum = round(length(X)*50), maxT = length(X)+5)
    gen.x[jj,] <- approx(myList$TList, myList$XList, xout = seq(0, max(length(X)-1, na.rm = TRUE), by=1), method = "constant", yleft = 0, yright = max(myList$XList))$y
  }
  mean.x <- colMeans(gen.x)

  X.update <- MH.XR.ind.gamma(r = RR,x = X,kix = K.i,km = theta.Y[3], thetax = theta.X, m = mean.x, useall = T, ptnum = 4)
  # X.update =MH.X(RR,X,kix=K.i,km=K.M, thetax=theta.X, ptnum = ptnum, useall = FALSE)
...
plot(tspan, mean.y); lines(tspan, sim.Y, col = "red");
...
plot(tspan, colMeans(result.X.trj)/mean(result.par[,2])); lines(tspan, sim.X/K.M)

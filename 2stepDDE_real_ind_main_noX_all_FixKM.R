...
for(rep in 2:nrepeat) {
  ...
  for(jj in 1:nsample){
    myListX <- TimeDelayGillespieforXR(A.X = theta[rep-1,1], B.X = B.X, alpha.X = theta[rep-1,3], beta.X = theta[rep-1,4], repnum = round(max.T*10000), maxT = max.T+2)
...
gen.y3 <- matrix(0, nrow = 3, ncol = max.T+1)
idx1 <- 200
for(jj in 1:3){
  myList2 <- TimeDelayGillespieforXY(A.X = theta[idx1,1], B.X = B.X, alpha.X = theta[idx1,3], beta.X = theta[idx1,4], A.Y = A.Y, B.Y = B.Y, alpha.Y = alpha.Y, beta.Y = beta.Y, K.M = theta[idx1,2], repnum = max.T*1000, maxT = max.T+3)
  sim.X2 <- approx(myList2$TList[!is.na(myList2$TList)], myList2$XList[!is.na(myList2$XList)], xout = seq(from = 0, to = max.T, by=1), method = "constant", yleft = 0, yright = max(myList2$XList[!is.na(myList2$XList)]))$y
...
par(mfrow = c(1,1))
matplot(Y.all, type = "l")
lines(rowMeans(Y.all), type ="l", col = "blue", lwd = 2)

...
setwd("/home/users/hphong/TimeDelay/Onestep_cross")
args=(commandArgs(TRUE))
fixBTF <- as.numeric(args[1]) 
...
short1 <- BDD(indata=Y.all,scale=1/1, B=0.05 * fixBTF,nrepeat = effrepeat, tun.B=300, tun.al = 0.5, tun.be = 70000)
save(short1, A.Y, K.M, file = paste("UsingOnestep_fixBTF", fixBTF,"_AX_", A.X, ".RData", sep = ""))

# short11<- MCMC.summary(result = short1, nrepeat=110000, burn=10000, thin=100,T,F,T)
# par(mfrow=c(1,1))
# matplot(short1, type ="l")
# selrow = seq(from = 500, by = 5, to = 1100)
